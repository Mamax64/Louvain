...
public class GraphBuilder {
  ...
  public Graph fromFile(String filename, boolean needsReIndexing) {
	filename = System.getProperty("user.dir") + "\\data\\" + filename;
    try {
  ...
  throws NumberFormatException, FileNotFoundException, IOException {
    ...
    while ((line = reader.readLine()) != null) {
      ...
      // si le poids n'a pas été adapté (ici plus le poids est élevé moins les deux classes sont liées, cette ligne permet d'inverser cela)
      //weight = 100 / weight;
  ...
  throws NumberFormatException, FileNotFoundException, IOException {
    ...
    while ((line = reader.readLine()) != null) {
      ...
      Double weightDouble = Double.parseDouble(splitLine[2]);
      if (weightDouble == 0.5) weightDouble = 1.0;
      int weight = weightDouble.intValue();
      // si le poids n'a pas été adapté (ici plus le poids est élevé moins les deux classes sont liées, cette ligne permet d'inverser cela)
      //weight = 100 / weight;
      weight = 100 / weight;
